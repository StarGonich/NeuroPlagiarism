...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (sum > T) {
            ...
            for (int j = max(t1[i] - 3600, 0); j <= t1[i]; j++) {
                ...
                sum += max(0, pref[r1] - pref[min(n, l + 1)]);
                if (sum > T) {
                    ans = min(ans, j);
                    break;
                }
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (sum > T) {
            ans = min(ans, j);
            int l1 = 0, r11 = 0;
            for (int j1 = max(j - 3600, 0); j1 <= j; j1++) {
                while (l1 < n && t2[l1] <= j1) {
                    l1++;
                }
                while (r11 + 1 < n && t1[r11 + 1] <= j1 + 3600) {
                    r11++;
                }
                sum = 0;
                if (l1 < n && t2[l1] <= j1 + 3600 && t2[l1] > j1) {
                    sum += min(t2[l1] - t1[l1], t2[l1] - j1);
                }
                if (r11 < n && t1[r11] <= j1 + 3600) {
                    sum += min(t2[r11] - t1[r11], j1 + 3600 - t1[r11]);
                }
                sum += max(0, pref[r11] - pref[min(n, l1 + 1)]);
                if (sum > T) {
                    ans = min(ans, j1);
                    break;
                }

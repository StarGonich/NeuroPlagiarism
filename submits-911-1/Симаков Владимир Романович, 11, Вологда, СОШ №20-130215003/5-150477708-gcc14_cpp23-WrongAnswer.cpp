...
void next_state(const Grid& config, Grid& next, int n, int m) {
    if (next.size() != static_cast<size_t>(n * m)) {
        next.resize(n * m);
    }
...
inline bool compare_grids(const Grid& a, const Grid& b) {
    return a == b;
}
...
int main() {
    ...
    vector<bool> reachable_flat(k * k, false);
    ...
    for (int i = 0; i < k; i++) {
        ...
        for (int step = 0; step < 2000 && !stable; step++) {
            ...
            for (int j = 0; j < k; j++) {
                int reach_idx = i * k + j;
                if (!reachable_flat[reach_idx]) {
                    if (compare_grids(next, configs[j])) {
                        reachable_flat[reach_idx] = true;
                    }
                }
            }

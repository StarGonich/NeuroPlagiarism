...
inline void solve() {
	...
	for (int x = 1; x <= 10000; ++x) {
		for (int y = 1; y <= 10000; ++y) {
			if (x * (a - c) + y * (b - d) == 1000 * (p - q) &&
				x * a + y * b == p * 1000 && x * c + y * d == 1000 * q) {
				ans.push_back(x);
				ans.push_back(y);
				if (ans.size() > 2) {
					cout << "Ambiguity";
	...
	if (ans.size() == 2 && ans[0] > 0 && ans[1] > 0) {
		cout << "Success\n";
		double x = ans[0] / (double)10;
		double y = ans[1] / (double)10;
		cout << fixed << setprecision(2) << x << ' ' << y;
		return;
	}
	else if (ans.size() == 0 && (a-c!=0 || b-d!=0 || (p-q) == 0)) cout << "Not positive";

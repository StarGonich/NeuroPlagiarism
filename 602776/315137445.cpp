...
void solve() {
	...
	q.push({ 0, cost[0] * 1000001 });
	while (q.size()) {
		auto p = q.top(); q.pop();

		int temp = p.second % 1000001;
		pair<long, long> frompair = { temp / 101, temp % 101 };
		temp = p.second / 1000001;
		pair<long, long> now = { temp / 101, temp % 101 };
		ll dpwas = -p.first;
		ll posnow = now.first;
		ll costnow = now.second;
		...
		for (auto& to : g[posnow][costnow]) {
			...
			q.push({ -costgo, ((topair.first * 101) + topair.second) * 1000001 + (posnow * 101) + costnow });
		}
	}
	...
	ans.pb({ n, 0 });
	while (from[now.first][now.second].second) {

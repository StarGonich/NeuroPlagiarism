...
void press(vector<vector<int>>& buf, int i, int j) {
    buf[i][j] = 1 - buf[i][j];
    buf[i - 1][j] = 1 - buf[i - 1][j];
    buf[i + 1][j] = 1 - buf[i + 1][j];
    buf[i][j - 1] = 1 - buf[i][j - 1];
    buf[i][j + 1] = 1 - buf[i][j + 1];
}
void change(vector<vector<int>>& buf, vector<int>& rr, int k, int& res) {
...
void press1(vector<vector<int>>& buf, int i, int j) {
    cout << i << ' ' << j << '\n';
    buf[i][j] = 1-buf[i][j] ;
    buf[i - 1][j] = 1-buf[i - 1][j];
    buf[i + 1][j] = 1-buf[i + 1][j];
...
void change1(vector<vector<int>>& buf, vector<int>& rr, int k, int& res) {
...
int32_t   main() {
    ...
    vector<vector<int>> mm(n + 2, vector<int>(n + 2)),un(n + 2, vector<int>(n + 2));
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= n; j++)
            cin >> matrix[i][j];
    for (int i = 1; i <= n; i++)
    ...
    for (int i = 0; i < (1ll << n); i++) {
        vector<vector<int>> buf_mm = mm;
        int buf_res = 0;
        change(buf_mm,rr, i, buf_res);
        for (int j = 2; j <=n; j++)
            for(int h=1;h<=n;h++)
            if (buf_mm[j - 1][h] == 1) {
                buf_res++;
    ...
    if (res <= res_un) {
        ...
        vector<vector<int>> buf_mm = un;
        int buf_res = 0;
        change1(buf_mm,rr, ind_un, buf_res);
        for (int j = 2; j <= n; j++)
            for (int h = 1; h <= n; h++)
                if (buf_mm[j - 1][h] == 1) {
                    press1(buf_mm, j, h);

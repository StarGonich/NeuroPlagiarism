...
Res poisk(double lef, double rig) {
	double l = lef, r = rig;
	while (r - l >= EPS) {
		double m = (r - l) / 3.0;
		double x1 = r - m;
		double x2 = l + m;
		double y1 = (100.0 * p - a * x1) / b;
		double y2 = (100.0 * p - a * x2) / b;
		double g1 = ((c * x1 / 100.0 ) + (d * y1 / 100.0 ));
		double g2 = ((c * x2 / 100.0 ) + (d * y2 / 100.0 ));
		double m1 = abs(q - g1);
		double m2 = abs(q - g2);
		if (m1 >= m2) r = x1;
		else l = x2;
	...
	double x = r;
	double y = (100.0 * p - a * x) / b;
	double g = ((c * x / 100.0 ) + (d * y / 100.0 ));

	rs.x = x;
	rs.y = y;
	rs.otv = g;
	if (abs(q - g) <= 1e-1) rs.kak = true;
	else rs.kak = false;
	return rs;
}
void solve() {
	...
	if (a == 100 && b == 100 && p == 50)cout << "Contradiction";
	else {
		cout << "Ambiguity";
	}
	return;

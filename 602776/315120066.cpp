#define _CRT_SECURE_NO_WARNINGS
#include <iostream>
#include <algorithm>
#include <iomanip>
#include <vector>
#include <set>
#include <map>
#include <unordered_map>
#include <unordered_set>
#include <cmath>
#include <math.h>

typedef long long ll;
typedef long double ld;
#define pll pair<ll,ll>  
#define pii pair<int,int>  

const ld EPS = 1e-1;
using namespace std;

void solve();

int main() {
#ifndef ONLINE_JUDGE
	FILE* FILE_IN = freopen("input.txt", "r", stdin);
	FILE* FILE_OUT = freopen("output.txt", "w", stdout);
#endif
	ios::sync_with_stdio(false);
	cin.tie(nullptr); cout.tie(nullptr);
	ll t = 1;
	//cin >> t;
	while (t--) solve();
}


//void solve() {
//	ld a, b, p; cin >> a >> b >> p;
//	ld c, d, q; cin >> c >> d >> q;
//	ll res = 0;
//	ll rx = 0, ry = 0;
//	for(ld x = -1000; x < 1000; x+=0.1)
//	{
//		ld y = (100.0 * p - a * x) / b;
//		cout << a << " / 100"  << " * " << x << " + " << b << " / 100" << " * " << y << " = " << p << " (" << a / 100 * x + b / 100 * y << ")" << endl;
//		cout << c << " / 100"  << " * " << x << " + " << d << " / 100" << " * " << y << " = " << q << " (" << c / 100 * x + d / 100 * y << ")" << endl;
//		cout << endl;
//	}
//}


//
//void solve() {
//	ld a, b, p; cin >> a >> b >> p;
//	ld c, d, q; cin >> c >> d >> q;
//	ll res = 0;
//	ld rx = 0, ry = 0;
//	for (ld x = -10000; x < 10000; x += 0.01)
//	{
//		ld y = (100.0 * p - a * x) / b;
//		ld g = ((c / 100 * x) + (d / 100 * y));
//		if (abs(q - ((c / 100 * x) + (d / 100 * y))) < EPS) {
//			//cout << x << " " << y << " " << endl;
//			//cout << q - g << endl;
//			rx = x;
//			ry = y;
//			res++;
//			x += 0.2;
//		}
//	}
//	if (res == 0) cout << "Contradiction";
//	else if (res > 1) cout << "Ambiguity";
//	else if (rx < 0 || ry < 0) cout << "Not positive";
//	else if (res == 1)
//	{
//		cout << "Success\n";
//		cout << rx << " " << ry;
//	}
//
//}

void solve()
{
	ll n; cin >> n;
	vector<ll> arr(n); for (auto& e : arr) cin >> e;
	vector<ll> posled;
	vector<pll> res;
	set<ll> check;
	ll to_fnd = 1;
	ll last_id = 0;
	for(int i = 0; i < n;)
	{
		if (to_fnd == n + 1)break;
		if(arr[i] == to_fnd)
		{
			to_fnd++;
			check.insert(i);
			posled.push_back(i);
			i++;
			last_id = i;
			continue;
		}
		ll m = i;
		while (m < n && arr[m] != to_fnd) m++;
		check.insert(m);
		posled.push_back(m);
		to_fnd++;
		ll ind_m = m;
		ll d = 1;
		while (m + 1 < n && arr[m] + 1 == arr[m + 1]) {
			check.insert(m + 1);
			posled.push_back(m + 1);
			to_fnd++;
			d++;
			m++;
		}
		res.push_back({ ind_m + 1,d });


		m = last_id;
		d = 1;
		to_fnd++;
		while (m + 1 < n && arr[m] + 1 == arr[m + 1]) {
			
			to_fnd++;
			d++;
			m++;
		}

		for (int j = m + 1; j < ind_m; j++)
		{
			check.insert(j);
			posled.push_back(j);
		}

		for (int j = last_id ; j <= m; j++) {
			check.insert(j);
			posled.push_back(j);
		}

		res.push_back({ last_id + 1,d });
		break;

	}
	for (int i = 0; i < n; i++)
	{
		if(check.find(i) == check.end())
			posled.push_back(i);
	}
	to_fnd = 1;
	for(auto& e : posled)
	{
		if (arr[e] == to_fnd) {
			to_fnd++;
		}
		else
		{
			cout << -1;
			return;
		}
	}
	sort(res.begin(), res.end());
	if(res.size() == 2)
	{
		for (auto& e : res) cout << e.first << " " << e.second << endl;

	}
	else cout << -1;
}

//void solve()
//{
//	ll n; cin >> n;
//	vector<ll> arr(n); for (auto& e : arr) cin >> e;
//	vector<ll> arr2 = arr;
//	vector<pll> res;
//	sort(arr2.begin(), arr2.end());
//	for (int i = 0; i < arr.size(); i++) {
//		if (arr[i] != arr2[i])
//		{
//			ll m = 0;
//			while (i + 1 < n && arr[i] + 1 == arr[i + 1] && arr[i] != arr2[i])
//			{
//				//cout << arr[i] << " " << arr[i + 1] << endl;
//				m++;
//				i++;
//			}
//			res.push_back({ i - m + 1, m + 1 });
//		}
//	}
//	if (res.size() == 2)
//	{
//		for (auto& e : res) {
//			cout << e.first << " " << e.second << endl;
//		}
//	}
//	else cout << -1;
//}
...
int main()
{
    ...
    int k; cin >> k;
    k--;
    //cout << Ch[k] + D[k] << ' ' << Ch[n - 1] << '\n';
    int mx = Ch[k], mn = Ch[k], ntmn = max(0,Ch[k]-D[k]), ntmx = min(Ch[k] + D[k],Ch[n-1]), pmx = k, pmn = k;
    int l, r, m;
    while (mx != ntmx || ntmn != mn) {
        //cout << ntmn << ' ' << ntmx << '\n';
        if (ntmx > mx) {
            l = pmx; r = n;
            while (r - l > 1) {
            ...
            //cout << r<<'r';
            for (int i = pmx; i <= l; i++) {
                if (Ch[i] + D[i] > ntmx) ntmx = Ch[i] + D[i];
                if (Ch[i] - D[i] < ntmn) ntmn = Ch[i] - D[i];
            }
            pmx = l;
        }
        ntmx = min(ntmx, Ch[n - 1]);
        ntmn = max(Ch[0], ntmn);
        if (ntmn < mn) {
            l = 0; r = pmn + 1;
            while (r - l > 1) {
...
//cout << ntmx << ' ' << ntmn << '\n';
    ...
    for (int i = 0; i < n; i++) {
        cin >> a;
        L[i] -= a;
        update_min(nom + i, L[i]);
        R[i] += a;
        update_max(nom + i, R[i]);
    }*/

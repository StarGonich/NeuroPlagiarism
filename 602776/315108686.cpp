...
void solve() {
	...
	ll posl = k;
	ll posr = k;
	while ((posl > 0 && x[posl] >= tl) || (posr <= n && x[posr] <= tr)) {
		while (posl > 0 && x[posl] >= tl) {
			tl = min(tl, x[posl] - d[posl]);
			tr = max(tr, x[posl] + d[posl]);
			posl--;
		}
		while (posr <= n && x[posr] <= tr) {
			tl = min(tl, x[posr] - d[posr]);
			tr = max(tr, x[posr] + d[posr]);
			posr++;
		}
	}

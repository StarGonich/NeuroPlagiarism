...
int main()
{
	...
	if (!a && !c) tolko_q2 = comp(b * q, p * d);
	if (!a && !c) nepr_tolko_q2 = !comp(b * q, p * d);
	if (!b && !d) tolko_q1 = comp(a * q, p * c);
	if (!b && !d) nepr_tolko_q1 = !comp(a * q, p * c);

	if (a && b && c && d) otl_v_k_raz = comp(a * d, b * c) && comp(b * q, p * d);
	if (a && b && c && d) nepr_otl_v_k_raz = comp(a * d, b * c) && !comp(b * q, p * d);

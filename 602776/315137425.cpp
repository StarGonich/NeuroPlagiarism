...
void inverse(vector<bitset<sz>>& v, int x, int y) {
    v[x][y] = 1 - v[x][y];
}

void change(vector<bitset<sz>>& v, int x, int y, int& ans, vector<pair<int, int>>& op) {
...
vector<pair<int, int>> calc(vector<bitset<sz>>& v, int col) {
    int ans = SMALL_INF;

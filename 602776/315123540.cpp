...
void press(vector<vector<int>>& buf, int i, int j) {
    buf[i][j] = (buf[i][j] + 1) % 2;
    buf[i-1][j] = (buf[i-1][j] + 1) % 2;
    buf[i+1][j] = (buf[i+1][j] + 1) % 2;
    buf[i][j-1] = (buf[i][j-1] + 1) % 2;
    buf[i][j+1] = (buf[i][j+1] + 1) % 2;
}
void change(vector<vector<int>>& buf, int k, int& res) {
    vector<int> rr(buf.size()-2);
...
void change1(vector<vector<int>>& buf, int k, int& res) {
    vector<int> rr(buf.size() - 2);
    for (int i = 0; i < rr.size(); i++)
    {
        res += k % 2;

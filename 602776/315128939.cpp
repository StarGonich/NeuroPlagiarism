...
vector<ll> hashes(string s) {
	...
	for (int i = 1; i < s.size(); i++) {
		hash[i] = hash[i - 1] + s[i] * fastPow(p, i, mod) + s[i] * fastPow(q, i, mod);
	}
...
string decode(string s) {
	...
	for (int i = 0; i < str.size(); i++) {
		for (int j = 0; j < 26; j++) {
			if (crushed_sum_hash - str[i] * fastPow(p, i, mod) - str[i] * fastPow(q, i, mod) + ('A' + j) * fastPow(p, i, mod) + ('A' + j) * fastPow(q, i, mod) == sum_hash) {
				str[i] = ('A' + j);
				return str;
			}
		}

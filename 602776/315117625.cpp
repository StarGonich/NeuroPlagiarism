...
void solve() {
	...
	vector<vector<tuple<int, int, int>>> pred(n, vector<tuple<int, int, int>>(101, { INF, INF, INF }));

	dp[0][a[0]] = 0;
	set<tuple<int, int, int>> st;
	st.insert({ 0, a[0], 0 });
	pred[0][a[0]] = { INF, INF, 0 };
	...
	while (!st.empty()) {
		...
		for (auto edge : g[v]) {
			...
			if (dp[to][min(a[to], mn)] > dp[v][mn] + cost) {
				...
				pred[to][min(a[to], mn)] = { v, mn, (a[to] < mn ? to :get<2>(pred[v][mn]))};
	...
	while(v != INF){
		...
		int cur = v;
		int mnn = mn;
		v = get<0>(pred[cur][mnn]);
		mn = get<1>(pred[cur][mnn]);
		if (v != INF) {
			int dist = (dp[cur][mnn] - dp[v][mn]) / a[get<2>(pred[v][mn])];
			need[get<2>(pred[v][mn])] += dist;
		}
	}

# Определяем входные данные — список транзакций и профили пользователей
...
def is_fraud(transaction, profile):
    # 1. Проверка IP-адреса: отличается ли от типичного для пользователя?
    if transaction["ip"] != profile["typical_ip"]:
        return True
    
    # 2. Проверка суммы: превышает ли в 3 раза средний чек?
    if transaction["amount"] > 3 * profile["avg_amount"]:
        return True
    
    # 3. Проверка времени: совершена ли транзакция ночью (00:00–06:00)?

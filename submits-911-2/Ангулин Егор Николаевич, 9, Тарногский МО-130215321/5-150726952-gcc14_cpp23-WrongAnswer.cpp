...
int main() {
    ...
    // Проверяем все возможные пороги
    for(int i = 0; i <= n; ++i) {
        // Текущий порог - это оценка следующего элемента (или 0, если все сообщения не фрод)
        int current_threshold = (i < n) ? messages[i].score : -1;
        
        // Обновляем счетчики для текущего порога
        if (i > 0) {
            // Учитываем предыдущий элемент
    ...
    // Если все сообщения не считаются фродом
    if (best_threshold == 1e9) {
        best_threshold = 0;
    }

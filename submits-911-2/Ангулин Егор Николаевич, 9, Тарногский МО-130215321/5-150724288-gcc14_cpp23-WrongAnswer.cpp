...
// Структура для хранения оценки и информации о фроде
struct Message {
    int score;
    bool is_fraud;
};
...
int main() {
    ...
    for(int i = 0; i < n; ++i) {
        ...
        // Проверяем, является ли текущая оценка уникальным порогом
        if(i == n-1 || messages[i].score != messages[i+1].score) {
            ...
            if(f1 > max_f1 || (f1 == max_f1 && messages[i].score < best_threshold)) {
                max_f1 = f1;
                best_threshold = messages[i].score;
            }
        }
    }
